...
namespace WeirdBrothers.ThirdPersonController
{
    public class WBThirdPersonController : NetworkBehaviour
    {
        ...
        [ClientRpc]
        internal void BreakCameraClientRPC(ulong clientID, Vector3 force)
        {
            if( IsOwner && OwnerClientId==clientID)
            {
                WBUIActions.EnableBrokenScreen?.Invoke(true);
                Context.Controller.setisKicked();
                Context.Controller.Rigidbody.linearVelocity = new Vector3(force.x /9f, force.y / 22f, force.z / 9f);
